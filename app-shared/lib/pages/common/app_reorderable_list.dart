//
// Orchid: This is the standard Flutter ReorderableListView with minor
// modifications to support a footer view.
//
...
/// The callback used by [AppReorderableListView] to move an item to a new
/// position in a list.
///
...
/// [AppReorderableListView] will need to account for this when inserting before
/// [newIndex].
///
...
/// {@youtube 560 315 https://www.youtube.com/watch?v=3fB1mxOsqJE}
// Orchid: Rename
class AppReorderableListView extends StatefulWidget {

  /// Creates a reorderable list.
  AppReorderableListView({
    this.header,
    this.footer,
  ...
  /// underlying list.
  ///
  /// This [AppReorderableListView] calls [onReorder] after a list child is dropped
  /// into a new position.
  final ReorderCallback onReorder;

  @override
  _AppReorderableListViewState createState() => _AppReorderableListViewState();
}
...
class _AppReorderableListViewState extends State<AppReorderableListView> {
  // We use an inner overlay so that the dragging list item doesn't draw outside of the list itself.
  final GlobalKey _overlayKey = GlobalKey(debugLabel: '$AppReorderableListView overlay key');
...
class _ReorderableListContentState extends State<_ReorderableListContent> with TickerProviderStateMixin<_ReorderableListContent> {
  ...
  // Orchid: Reduce default drop extent
      ...
      return SingleChildScrollView(
        ...
        child: _buildContainerForScrollDirection(
          children: <Widget>[
            ...
            // Orchid: Show the final drop target only when dragging
            if (widget.reverse && _dragging != null) _wrap(finalDropArea, widget.children.length, constraints),
            for (int i = 0; i < widget.children.length; i += 1) _wrap(widget.children[i], i, constraints),
            // Orchid: Show the final drop target onlyl when dragging
            if (!widget.reverse && _dragging != null) _wrap(finalDropArea, widget.children.length, constraints),
            // Orchid: Apply the footer
