...
class PathTrim {
  ...
  /// Path trimming utility.
  /// Note: Remove this if and when path trimming is incorporated into Flutter.
  /// Source: https://github.com/2d-inc/Flare-Flutter/blob/master/flare_flutter/lib/trim_path.dart
  static Path trim(Path path, double startT, double stopT, bool complement,
      bool isSequential) {
    if (isSequential) {
  ...
  static double _appendPathSegmentSequential(
  ...
  static void _appendPathSegmentSync(
  ...
  static Path _trimPathSequential(
    ...
    if (complement) {
      if (trimStart > 0.0) {
        offset = _appendPathSegmentSequential(
            metrics, result, offset, 0.0, trimStart);
      }
  ...
  static Path _trimPathSync(Path path, double startT, double stopT, bool complement) {
    final Path result = Path();
