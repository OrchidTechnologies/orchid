      ...
      seed:
        ...
        for (unsigned p(0); p != 4; ++p)
            for (unsigned d(0); d != (p+1)*2+1; ++d) {
                ...
                const auto positive(21000+1835 +(3000+800+20000+800+7984)*p+(p==0?0:12+7400+4437) +(512+800+5000+265)*d+(d==0?0:12));
        ...
        co_await Audit("claim1(1,0)", co_await endpoint_.Send(provider_, indirect, minimum_, claim1(Zero<32>(), where(), payment(), args...)), 66319);
        co_await check(-1);

        co_await Audit("claim1(1,0)", co_await endpoint_.Send(provider_, lottery, minimum_, claim1(Zero<32>(), where(), payment(), args...)), 66319);
        co_await check(-1);

        co_await Audit("claim1(1,1)", co_await endpoint_.Send(provider_, lottery, minimum_, claim1(refund(), where(), payment(), args...)), 57646);
        co_await check(-1);
