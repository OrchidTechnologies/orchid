...
if test -L "${ANDROID_HOME}"/ndk-bundle; then
    ...
    "${ANDROID_HOME}"/cmdline-tools/latest/bin/sdkmanager --uninstall "ndk-bundle" >/dev/null
fi

echo y | "${ANDROID_HOME}"/cmdline-tools/latest/bin/sdkmanager "build-tools;29.0.2" "ndk;25.1.8937393" "platforms;android-33" >/dev/null
