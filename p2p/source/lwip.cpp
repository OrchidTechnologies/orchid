...
int LwipSocket::SetOption(Option opt, int value) {
  ...
  if (sopt == IPV6_TCLASS) {
    ...
    ::lwip_setsockopt(s_, IPPROTO_IP, IP_TOS, (SockOptArg)&value, sizeof(value));
  }
#endif
  int result =
      ::lwip_setsockopt(s_, slevel, sopt, (SockOptArg)&value, sizeof(value));
  if (result != 0) {
    UpdateLastError();
  }
  return result;
...
int LwipSocket::Recv(void* buffer, size_t length, int64_t* timestamp) {
  ...
  if ((received == 0) && (length != 0)) {
    ...
    // Must turn this back on so that the lwip_select() loop will notice the close
    // event.
...
bool SocketDispatcher::IsDescriptorClosed() {
  ...
  do {
    res = ::lwip_recv(s_, &ch, 1, MSG_PEEK);
...
class Signaler : public Dispatcher {
 ...
 private:
  LwipSocketServer* const ss_;
  std::array<int, 2> afd_;
...
void LwipSocketServer::Add(Dispatcher* pdispatcher) {
  ...
  if (key_by_dispatcher_.count(pdispatcher)) {
    RTC_LOG(LS_WARNING)
        << "LwipSocketServer asked to add a duplicate dispatcher.";
    return;
  }
...
static void ProcessEvents(Dispatcher* dispatcher,
  ...
  if (check_error) {
    ...
    int res = ::lwip_getsockopt(dispatcher->GetDescriptor(), SOL_SOCKET, SO_ERROR,
                           &errcode, &len);
    if (res < 0) {
...
// http://crbug.com/344505
  ...
  while (fWait_) {
    // Zero all fd_sets. Although lwip_select() zeros the descriptors not signaled,
...
void LwipSocketServer::AddEpoll(Dispatcher* pdispatcher, uint64_t key) {
...
void LwipSocketServer::UpdateEpoll(Dispatcher* pdispatcher, uint64_t key) {
  RTC_DCHECK(epoll_fd_ != INVALID_SOCKET);
  int fd = pdispatcher->GetDescriptor();
  RTC_DCHECK(fd != INVALID_SOCKET);
  if (fd == INVALID_SOCKET) {
